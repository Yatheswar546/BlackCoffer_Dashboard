        ...
        if (ajax_request.readyState == 4 && ajax_request.status == 200) {
            ...
            if (response.data.length > 0) {
                for (var count = 0; count < response.data.length; count++) {
                    ...
                    html += '<td>' + response.data[count].pestle + '</td>';
...
function fetchPestles(callback) {
    fetch('http://localhost/blackCoffer/api/pestle.php', {
        method: 'GET',
        headers: {
...
function initializeTopicDropdown(pestles) {
    ...
    if (pestles && pestles.data && pestles.data.length > 0) {
        pestles.data.forEach((pestle) => { // Use topics.data here
            ...
            checkbox.name = 'pestle';
            checkbox.value = pestle.pestle;
            checkbox.id = 'check';

            const topicText = document.createTextNode(pestle.pestle);
...
fetchPestles(initializeTopicDropdown);

/////////////////////// FETCH RECORDS BY COUNTRY /////////////////////////////
$(document).ready(function () {
    $('#findPestle').click(function (event) {
        event.preventDefault();
        var action = 'searchPestle';
        var arr = [];
