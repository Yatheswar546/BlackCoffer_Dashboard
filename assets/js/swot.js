        ...
        if (ajax_request.readyState == 4 && ajax_request.status == 200) {
            ...
            if (response.data.length > 0) {
                for (var count = 0; count < response.data.length; count++) {
                    ...
                    html += '<td>' + response.data[count].swot + '</td>';
...
function fetchSwots(callback) {
    fetch('http://localhost/blackCoffer/api/swot.php', {
        method: 'GET',
        headers: {
...
function initializeTopicDropdown(swots) {
    const topicDropdown = document.querySelector('.dropdown-menu-swot');

    if (swots && swots.data && swots.data.length > 0) {
        swots.data.forEach((swot) => { // Use topics.data here
            ...
            checkbox.name = 'swot';
            checkbox.value = swot.swot;
            checkbox.id = 'check';

            const topicText = document.createTextNode(swot.swot);
...
fetchSwots(initializeTopicDropdown);

/////////////////////// FETCH RECORDS BY SWOTS /////////////////////////////
$(document).ready(function () {
    $('#findSwots').click(function (event) {
        event.preventDefault();
        var action = 'searchSwots';
        var arr = [];
        $.each($("input[name='swot']:checked"), function () {
            arr.push($(this).val());
        });
        // alert(arr);
        // console.log(arr);
