...
function fetchSectors(callback) {
    fetch('http://localhost/blackCoffer/api/sectors.php', {
        method: 'GET',
        headers: {
...
function initializeTopicDropdown(sectors) {
    ...
    if (sectors && sectors.data && sectors.data.length > 0) {
        sectors.data.forEach((sector) => { // Use topics.data here
            ...
            checkbox.value = sector.sector;
            checkbox.id = 'check';

            const topicText = document.createTextNode(sector.sector);
...
fetchSectors(initializeTopicDropdown);

/////////////////////// FETCH RECORDS BY SECTORS /////////////////////////////
$(document).ready(function () {
    $('#findSectors').click(function (event) {
        event.preventDefault();
        var action = 'searchSectors';
        var arr = [];
